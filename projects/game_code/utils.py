...
def load_image(
    ...
    rotate_deg=0,
    squre=0,
    hflip=False     # ⬅ 좌우 반전 여부
):
    """
    path      : 이미지 경로
    diameter  : 최종 리사이즈 크기 (정사각형)
    do_crop   : True면 자동 크롭 후 리사이즈
    rotate_deg: 시계반대 방향 회전 각도
    squre     : True면 정사각형, False면 세로 2배 비율
    hflip     : True면 좌우 반전 적용
    """

    # 1️⃣ 이미지 열기 + EXIF 자동 방향 보정
    img = Image.open(path)
    ...
    if squre == 0:
        ...
        img = img.resize((diameter, diameter * 2), Image.LANCZOS)
    else:
        img = img.resize((diameter, diameter * 3), Image.LANCZOS)
